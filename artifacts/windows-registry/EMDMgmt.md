...
location:
  ...
  acquisition-notes: |
    Live system: SOFTWARE hive is open and NOT copyable via simple file copy —
...
- name: volume-identifier-string
  ...
  - {concept: VolumeLabel, role: deviceLabel}
  - {concept: FilesystemVolumeSerial, role: runtimeSerial}
  note: |
    Canonical format (Vista–11):
...
- name: volume-serial-number
  ...
  encoding: "decimal (NOT hex) — EMDMgmt stores the FS VSN as a decimal integer (uint32). Other artifacts (LNK, Prefetch, Partition/Diagnostic Vbr0) store it hex. Convert for cross-artifact join: dec→hex, zero-pad to 8 chars, format XXXX-XXXX."
  note: |
    Common parser pitfall. Example: EMDMgmt value `3472651852` converts to
...
- name: cache-status
  ...
  encoding: uint32
  note: |
    ReadyBoost eligibility/configuration status for this volume.
...
- name: physical-device-size
  ...
  note: |
    Byte-accurate device capacity. Cross-references Partition/Diagnostic-1006
...
- name: key-last-write
  ...
  note: |
    Subkey last-write time corresponds to when ReadyBoost last updated
...
- proposition: CONNECTED
  ceiling: C3
  note: |
    EMDMgmt subkey existence + key-last-write establishes "Windows evaluated
...
anti-forensic:
  ...
  audit-trail: |
    SOFTWARE hive transaction logs (SOFTWARE.LOG1, SOFTWARE.LOG2) retain
